...
export default function Persone({ last_message, chat }){
  ...
  const last_message_text = last_message.text ? last_message.text.slice(0,20)+ last_message.text.length>20 && "..." : "Start new chat."
    ...
    <div onClick={()=>Navigate("/inbox",{ state:{chat} })} className="hover:scale-95 transition flex justify-between items-center p-3" key={chat.user_name}>
        <div className="flex gap-3 items-center">
          ...
          <div className="flex flex-col ">
            <p>{user_name}</p>
            <p className={`${!last_message.seen && !isMyMessage ? "font-bold" :"text-[#84d9d5]"} text-sm`}>{ typing === true ? "Typing..." : last_message_text }</p>
          </div>
        </div>
        {
